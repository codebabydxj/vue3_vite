...
interface UploadFileProps {
  ...
  request?: { url: string, method: string } | any; // 请求api包含 url和 method ==> 非必传，没传默认走统一上传接口
...
const handleHttpUpload = async (options: UploadRequestOptions) => {
  ...
  try {
    ...
    if (props.request && props.request.url) {
      res = props.request.method == 'post' ? await client.post(props.request.url, formData) : await client.get(props.request.url, formData)
    } else {
      res = await client.post(uploadFiles, formData)
    }
    emit('update:imageUrl', res.data.fileUrl);
