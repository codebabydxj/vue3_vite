<template>
  <flex-card>
    <div class="base-warp">
      <div class="error">
        <Transition name="slide-icon">
          <svg-icon v-if="show" name="403" :iconStyle="{ width: '252px', height: '294px'}" />
        </Transition>
        <div class="ml-12">
          <Transition :duration="800" name="slide-num">
            <p v-if="show" class="font-medium text-4xl">403</p>
          </Transition>
          <Transition :duration="1200" name="slide-msg">
            <p v-if="show" class="text-msg">抱歉，你无权访问该页面 🙅‍♂️🙅‍♀️</p>
          </Transition>
          <Transition :duration="1400" name="slide-btn">
            <el-button v-if="show" type="primary" @click="goBack">返回首页</el-button>
          </Transition>
        </div>
      </div>
    </div>
  </flex-card>
</template>

<script setup lang="ts" name="page403">
import { ref, onMounted } from "vue"
import { useRouter } from "vue-router"

const router = useRouter();
const show = ref(false)

onMounted(() => {
  show.value = true
})
const goBack = async () => {
  router.push('/');
}
</script>

<style scoped lang="scss">
@import "./index.scss";
</style>