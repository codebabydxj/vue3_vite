...
export default defineComponent({
  ...
  setup(_, context) {
    const { slots } = context;
    const getScopedSlot = (slotName: any) => {
      if (!slotName) {
        return null;
      }
    ...
    const getSlotValue = (slot: any, scopedData: any, defaultNode: any = null) => {
      if (isFunction(slot)) {
  ...
  render() {
    ...
    while (currentNode) {
      ...
      if (currentNode.level === 1 && !currentNode.parent) {
        // el-tree-v2的第一层node是没有parent的，必需将 treeData 创建一个parent
        if (!this.treeData || !Array.isArray(this.treeData)) {
          throw Error(
            "如果你使用element-plus 的 el-tree-v2（Virtualized tree），则 element-tree-line 需要数据"
          );
        }
        parentNode = {
